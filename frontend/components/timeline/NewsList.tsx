      ...
      <div className={cn("relative", isSlider && "group")}>
        ...
        {isSlider && (
          <>
            ...
            <div
              className={cn(
                "pointer-events-none absolute inset-y-0 left-0 w-14 bg-linear-to-r from-neutral-50 to-transparent transition-opacity duration-200",
                canScrollLeft ? "opacity-100" : "opacity-0"
              )}
            />
            <div
              className={cn(
                "pointer-events-none absolute inset-y-0 right-0 w-14 bg-linear-to-l from-neutral-50 to-transparent transition-opacity duration-200",
            ...
            {canScrollLeft && (
              <button
                ...
                className="absolute top-1/2 left-1 flex size-8 -translate-y-1/2 cursor-pointer items-center justify-center rounded-full border border-neutral-200 bg-white text-neutral-500 opacity-0 shadow-sm transition-opacity duration-200 group-hover:opacity-100 hover:border-neutral-300 hover:text-neutral-700"
              >
                <ChevronLeft size={16} />
              </button>
            )}
            {canScrollRight && (
              <button
                ...
                aria-label="다음 뉴스"
                onClick={() => scrollByCard(1)}
                className="absolute top-1/2 right-1 flex size-8 -translate-y-1/2 cursor-pointer items-center justify-center rounded-full border border-neutral-200 bg-white text-neutral-500 opacity-0 shadow-sm transition-opacity duration-200 group-hover:opacity-100 hover:border-neutral-300 hover:text-neutral-700"
              >
