# seed_cpi.py
# FRED CPI(CPIAUCSL)를 1회 가져와서 Supabase calendar_events에 저장하는 수동 스크립트.
# main.py 스케줄러에는 아직 연결하지 않는다 - 기본 흐름이 정상 동작한 뒤 별도로 자동화할 예정.
#
# 실행: backend/ 디렉토리에서 `uv run python scripts/seed_cpi.py`
...
async def main() -> None:
    event = await calendar_service.ingest_cpi_from_fred()
    print(
