...
#
# 주의: 지금은 DB를 거치지 않고 요청이 올 때마다 KIS·네이버를 새로 호출한다.
# 그래서 응답이 몇 초 걸리고, 호출할 때마다 값이 조금씩 달라진다(장중이면 특히).
# 저장 로직이 붙으면 DB에서 읽는 방식으로 바뀐다
# with_briefing=false를 붙이면 LLM 단계를 건너뛴다 (예: /timeline/slot/0730?with_briefing=false)
# LLM을 두 번 부르면 1분 안팎이 걸려서, 수집 결과만 빠르게 볼 때 쓴다
