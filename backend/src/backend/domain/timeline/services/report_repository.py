...
async def save_report_content(session: AsyncSession, report_type: str, start_date: date, content: dict) -> TimelineReport:
    ...
    report.sections = [
        TimelineReportSection(
            ...
            points=[TimelineReportPoint(seq=point_seq, body=body) for point_seq, body in enumerate(section["points"], start=1)],
        )
        for seq, section in enumerate(content["sections"], start=1)
    ]
    report.keywords = [TimelineReportKeyword(seq=seq, title=item["title"], description=item["description"]) for seq, item in enumerate(content["keywords"], start=1)]
