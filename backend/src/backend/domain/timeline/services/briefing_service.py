...
def _fact_subtitle(time_slot: str, indicators: list[dict], sectors: list[dict], top_gainers: list[dict], intraday_changes: list[dict]) -> str | None:
    ...
    if time_slot in ("08:30", "17:30", "20:00") and top_gainers:
        prefix = "" if time_slot == "08:30" else "전일 종가 대비 "
        return prefix + _joined_rates([(row["name"], row["change_rate"]) for row in top_gainers])
...
def _fact_point(time_slot: str, indicators: list[dict], sectors: list[dict], top_gainers: list[dict], intraday_changes: list[dict]) -> dict | None:
    ...
    if time_slot == "07:30" and indicators:
        ...
        body = f"{basis}에서 전일 종가 대비 등락률이 가장 높은 종목은 {stocks}입니다." if time_slot == "08:30" else f"{basis} 전일 종가 대비 등락률이 가장 높은 종목은 {stocks}입니다."
...
def _check(time_slot: str, data: str, briefing: dict, guides: list[dict]) -> tuple[dict, list[dict]]:
    ...
    if reviewed["briefing"]["subtitle"].rstrip(". ").endswith(("니다", "어요", "해요")):
        logger.warning("%s 브리핑 부제가 문장형입니다 (저장 후 확인 필요): %s", time_slot, reviewed["briefing"]["subtitle"])
    return (
        {
