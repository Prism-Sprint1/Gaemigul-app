...
# 두지 않고 core에 모아서 전역으로 쓴다(2026-09-10). 현재는 calendar 도메인에서 기업 실적
# 발표일 소스로 쓸 수 있는지 테스트하는 단계라, 이 파일은 조회만 하고 아무 것도 DB에 저장하지
# 않는다.
#
...
def get_disclosure_list(
    ...
    *,
    ...
    while True:
        ...
        response = httpx.get(f"{_BASE_URL}/list.json", params=params)
        response.raise_for_status()
        body = response.json()
...
# report_nm에 이 키워드가 있으면 삼성전자의 분기별 잠정실적 공시다(2단계 조사에서 실제 라이브
# 데이터로 확인 - "연결재무제표기준영업(잠정)실적(공정공시)" 패턴 하나뿐이고, 정정 공시는 앞에
# "[기재정정]"만 붙는다). 다른 회사도 이 문구를 그대로 쓰는지는 아직 확인 전이다.
_PRELIMINARY_EARNINGS_KEYWORD = "영업(잠정)실적"
...
def get_key_accounts(corp_code: str, bsns_year: str, reprt_code: str = "11011") -> dict:
    settings = get_settings()
    response = httpx.get(
        f"{_BASE_URL}/fnlttSinglAcnt.json",
        params={
            "crtfc_key": settings.dart_api_key,
            "corp_code": corp_code,
            "bsns_year": bsns_year,
